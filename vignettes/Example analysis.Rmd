...
ggplot(long_data, aes(x = expression)) +
  ...
  labs(
    ...
    subtitle = str_wrap(paste0("Two histograms are shown with the highest correlated gene, ", g_max,", with a 
                               mean correlation of ~",round(mean(long_data[long_data$gene == g_max[1],]$expression), 4),
                               ", and the lowest correlated gene, ",g_min," with a mean correlation of
                               ~",round(mean(long_data[long_data$gene == g_min[1],]$expression), 4),
                               ". However, note the large difference in distributions."), width = 95),
    caption = "Data Source: Kidney Consortium database"
  )  +
  theme(axis.text.y = element_blank(), axis.ticks.y = element_blank(), plot.title = 
          element_text(face = "bold", hjust = 0.5), axis.text.x = element_text(size = 10))
