...
ggplot(data = data.frame(cors), aes(x = cors)) + 
  ...
  labs(
    x = "Correlation",
    y = "Count",
    title = paste0("Mean Correlation between Red Channel and Gene Expression"),
...
ggplot() + 
  ...
  annotate("text", x = -Inf, y = -Inf, label = sprintf("R: %.2f", corr_coefficient), 
...
ggplot(long_data, aes(x = expression)) +
  ...
  labs(
    ...
    subtitle = str_wrap(paste0("Two histograms are shown with the highest correlated gene,", g_max,", and the 
                                lowest correlated gene, ",g_min,". The histogram is displaying the mean correlation
                                per tile within the tissue image."), width = 100),
    caption = "Data Source: Kidney Consortium database"
  )  +
  theme(axis.text.y = element_blank(), axis.ticks.y = element_blank(), plot.title = 
          element_text(face = "bold", hjust = 0.5), axis.text.x = element_text(size = 10))
